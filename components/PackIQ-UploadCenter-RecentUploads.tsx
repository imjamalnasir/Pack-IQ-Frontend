...
import { FileBox, FileChartColumn, FileSpreadsheet } from "lucide-react"
...
export function PackIQUploadCenterRecentUploads() {
  ...
  const uploadTypeClass = (type: string) => {
  ...
  const getFileName = (row: RecentDocument) => row.originalFileName ?? "—"
  const getType = (row: RecentDocument) => normalizeDocType(row.docType)
  const getDateTime = (row: RecentDocument) => formatDateTime(row.createdAt)
  const getStatus = (row: RecentDocument) => normalizeStatus(row.status)
            ...
            <TableBody>
              {items.map((row, i) => (
                <TableRow key={getRowId(row, i)}>
                  <TableCell>{getFileName(row)}</TableCell>
                  <TableCell>
                    <Badge variant="outline" className={uploadTypeClass(row.docType ?? "")}>
                      {getType(row)}
                    </Badge>
                  </TableCell>
